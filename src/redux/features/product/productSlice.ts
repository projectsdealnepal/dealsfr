...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    makeSelectedProductList(
      state,
      action: PayloadAction<AddProductOnDiscount>
    ) {
      const productIds = Object.keys(action.payload.items).map(Number);
      if (action.payload.rowId) {
        const product = state.productData?.results.find(
          (item: ProductItem) =>
            (item as ProductItem).id === action.payload.rowId
        );
        if (product) state.selectedProductList.push(product);
      }
      if (!action.payload.rowId) {
    ...
    addDiscountedProductList(
      state,
      action: PayloadAction<DiscountedProductType[]>
    ) {
    ...
    setDiscountAppliedProductList(
      state,
      action: PayloadAction<AddProductOnDiscountPayload[]>
    ) {
    ...
    updateTempAddedProduct(
      state,
      action: PayloadAction<Partial<GenericProductItem>>
    ) {
    ...
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<string | undefined>) => {
          ...
          state.productLoading = false;
          state.productError =
            action.payload || "Failed to fetch filter products";
        }
      )
      ...
      .addCase(
        addProductsToStore.rejected,
        (state, action: PayloadAction<string | undefined>) => {
          state.addProductToStoreError =
            action.payload || "Failed to add product to store";
          state.addProductToStoreLoading = false;
        }
      )
      ...
      .addCase(
        updateBulkProducts.rejected,
        (state, action: PayloadAction<string | undefined>) => {
          state.bulkProductUpdateError =
