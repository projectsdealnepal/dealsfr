...
import { Plus, Trash2, AlertCircle, PlusCircle } from 'lucide-react';
...
type DiscountItem = PercentageDiscount | FixedAmountDiscount | BOGODiscount | SpendGetDiscount;
...
const DiscountManager: React.FC = () => {
  ...
  const [selectedTargetType, setSelectedTargetType] = useState<TargetType>("PRODUCT")
      ...
      switch (currentItem.discountType) {
        case 'PERCENTAGE':
          percentageSchema.parse({
            percentageValue: (currentItem as PercentageDiscount).percentageValue,
          });
          break;
        case 'FIXED_AMOUNT':
        ...
        discountItems.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {discountItems.length} discount {discountItems.length === 1 ? 'rule' : 'rules'} configured
              </CardDescription>
            </CardHeader>
                  ...
                  <AlertDescription className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="font-medium">
                        {discountTypeOptions.find(opt => opt.value === item.discountType)?.label}
                      </div>
                      <div className="text-sm text-muted-foreground mt-1">
                        {renderDiscountSummary(item)} • Applies to: {item.targetType.toLowerCase()}
                      </div>
                    </div>
                    <Button
                      variant="ghost"
